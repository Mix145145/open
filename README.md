...
- **Настройка кадра**: в меню сканирования отправляет принтер в центр стола (X54 Y110) на заданную высоту Z и открывает окно живого предпросмотра.
- **FOV-профили по качеству**: в окне настройки можно переключать `FHD`/`2K`/`4K`, вводить FOV X/Y (мм) и сохранять отдельно для каждого режима.
- **FOV = Move**: общий параметр в меню сканирования. Если включён — шаг равен FOV и используется простое склеивание с мягким размытием швов; если выключен — шаг меньше FOV (перекрытие) и применяется MultiBand Blender.
